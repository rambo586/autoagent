...
一个能先用起来的多 Agent 开发闭环：你只提需求，Claude CLI（可使用你现有的 DeepSeek 后端配置）负责总控，Cursor CLI 负责主力开发，Codex CLI 负责规划、测试和只读复审。
...
macOS 需要 Python 3.10+、Git、`tmux`、`uv`，以及已经登录可用的 `claude`、`cursor-agent`（或 `agent`）和 `codex`。
...
# 先做无消耗检查；加 --live 会真实调用三个客户端并消耗少量额度
autoagent doctor
...
1. Manager 把原始需求交给 Planner，得到结构化计划和验收标准。
2. Developer 在隔离 worktree 中实现；遇到疑义必须向 Manager 发送澄清请求。
3. Tester 运行项目测试，并对每条验收标准给出证据。
4. 测试不通过就回到 Developer，最多循环 3 次；通过后交给 Reviewer 独立复审。
5. Manager 写入 `final-report.md`。涉及产品取舍、凭据、部署或破坏性操作时，任务标记为 `BLOCKED`，保留现场等待人工决定。
